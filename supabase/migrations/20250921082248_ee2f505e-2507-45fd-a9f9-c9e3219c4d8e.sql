-- Supprimer complètement le système admin_users et utiliser uniquement Supabase Auth

-- 1. Supprimer les policies liées à admin_users
...
-- 2. Supprimer les fonctions liées à admin_users
DROP FUNCTION IF EXISTS public.is_admin_authenticated();
DROP FUNCTION IF EXISTS public.verify_admin_access();
DROP FUNCTION IF EXISTS public.bootstrap_admin_if_none_exists();
DROP FUNCTION IF EXISTS public.create_admin_auth_user(text, text, text);
DROP FUNCTION IF EXISTS public.create_admin_user(extensions.citext, text, text, boolean);
DROP FUNCTION IF EXISTS public.change_own_password(text, text);
DROP FUNCTION IF EXISTS public.create_first_super_admin(extensions.citext, text, text);
...
DROP FUNCTION IF EXISTS public.secure_admin_login(text, text, inet);
DROP FUNCTION IF EXISTS public.change_admin_password(text, text, uuid);
DROP FUNCTION IF EXISTS public.is_admin();
DROP FUNCTION IF EXISTS public.is_admin(uuid);
DROP FUNCTION IF EXISTS public.is_admin_uuid(uuid);

-- 3. Créer une fonction simple pour vérifier les admins via auth.users
...
AS $$
  -- Pour simplifier, on considère que tous les utilisateurs auth.users sont des admins
  -- ou on peut filtrer par email si nécessaire
  SELECT auth.uid() IS NOT NULL;
$$;

-- 4. Recréer les policies RLS avec la nouvelle fonction simple
...
-- 5. Supprimer la table admin_users
