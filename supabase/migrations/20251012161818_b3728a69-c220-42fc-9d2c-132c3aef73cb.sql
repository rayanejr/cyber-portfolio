-- CORRECTION COMPLÈTE ET FINALE DES TRIGGERS
...
DROP TRIGGER IF EXISTS log_security_event_trigger ON skills;
DROP TRIGGER IF EXISTS log_security_event_trigger ON formations;
DROP TRIGGER IF EXISTS log_security_event_trigger ON tools;

-- 2. Supprimer l'ancienne fonction
DROP FUNCTION IF EXISTS public.log_security_event_trigger() CASCADE;

-- 3. Créer la fonction CORRECTE avec les bonnes valeurs de severity
...
-- 4. Recréer les triggers sur TOUTES les tables
